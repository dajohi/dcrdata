...
function buildProgressBar (data) {
  ...
  const htmlString = data.bar_msg.length > 0 ? clean(`<p style="font-size:14px;">${data.bar_msg}</p>`) : "";
  const subtitle = data.subtitle.trim();
  let notifStr = subtitle.length > 0 ? clean(`<span style="font-size:11px;">notification : <i>${subtitle}</i></span>`) : "";
