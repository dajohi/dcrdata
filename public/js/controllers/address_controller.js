...
function formatter (data) {
  let html = this.getLabels()[0] + ": " + ((data.xHTML === undefined) ? "" : data.xHTML);
...
export default class extends Controller {
  ...
  async fetchTable (txType, count, offset) {
    ...
    const settings = ctrl.listSettings;
    settings.n = count;
  ...
  drawGraph () {
    const ctrl = this;
    const settings = ctrl.chartSettings;
  ...
  validGraphInterval (interval) {
    const bin = interval || this.chartSettings.bin || this.activeBin;
    let b = false;
  ...
  validateZoom (binSize) {
    ...
    const zoom = Zoom.validate(ctrl.activeZoomKey || ctrl.chartSettings.zoom, ctrl.xRange, binSize);
    ctrl.setZoom(zoom.start, zoom.end);
    ctrl.graph.updateOptions({
  ...
  setFlowChecks () {
    const bitmap = this.chartSettings.flow;
  ...
  setChartType () {
    const chart = this.chartSettings.chart;
